...
class UserController extends Controller
{
    ...
    public function store(StoreUserRequest $request)
    {
        ...
        if ($files = $request->file('profile_photo')) {
            // Define upload path
                $destinationPath = public_path('/profile_photos/'); // upload path
      // Upload Orginal Image
                $profileImage = date('YmdHis') . "." . $files->getClientOriginalExtension();
                $files->move($destinationPath, $profileImage);

                $profile_photo = "$profileImage";
                //$request->profile_photo = "$profileImage";
        }
        $this->userRepository->create($request->all(),$profile_photo);
    ...
    public function update(UpdateUserRequest $request, User $user)
    {
      ...
      // Upload Orginal Image
        ...
        $this->userRepository->update($user, $request->all(),$profile_photo);
